...
class Settings:
    ...
    # DATA_FILE 基于 config.py 位置计算绝对路径，不依赖 cwd
...
**注意：** 配置通过 `.env` 文件注入（`python-dotenv`），P1 已提供 `.env.example` 模板。各角色在自己的 `backend/.env` 中配置本地值。`DATA_FILE` 使用基于 `config.py` 文件位置的绝对路径，无论从哪个目录启动服务器都能正确找到 `data/store.json`。
...
# Windows 下 Python 的 mimetypes 不认 .js 文件，浏览器会拒绝加载 ES Module
# 必须在挂载 StaticFiles 之前注册 MIME 类型
...
# 用 admin / admin123456 登录，应跳转到管理后台
```

**P1 额外创建的占位页面：**

为了让登录后跳转不 404，P1 创建了 3 个最小占位页面（仅含导航栏 + 提示文字，后续角色替换为完整页面）：

| 文件 | 对应角色 | 后续负责 |
|------|----------|----------|
| `frontend/pages/admin/stats.html` | admin 登录后跳转 | P4 替换 |
| `frontend/pages/handler/dashboard.html` | handler 登录后跳转 | P2 替换 |
| `frontend/pages/internal/approval.html` | approver 登录后跳转 | P3 替换 |

**P1 提供的 `.env.example` 模板：**

`backend/.env.example`（已提交 Git）包含所有配置项的默认值。各角色首次启动前执行 `cp .env.example .env` 即可。`.env` 在 `.gitignore` 中，不会被提交。

> **P1 里程碑：** 项目骨架 + 认证模块完成。`feature/auth-infra` 分支已推送。其他 4 人 clone 后 `git checkout feature/auth-infra` 查看，基于 `main` 建自己的分支开始并行开发。
